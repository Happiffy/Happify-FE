      ...
      <section className="grid min-w-0 gap-5" aria-labelledby="community-title">
        <form className={`${card} sticky top-5 z-20 grid min-w-0 gap-3 p-3 sm:p-5`} onSubmit={shareCommunityPost}>
          <h2 id="community-title" className="sr-only">Anonymous community</h2>
           <div className="grid gap-3 sm:grid-cols-[220px_minmax(0,1fr)]"><input className="min-h-12 rounded-2xl border-2 border-[#E5E5E5] px-4 font-bold outline-none focus:border-[#CE82FF] focus:ring-4 focus:ring-[#F3DFFF]" value={communityAlias} maxLength={60} onChange={(event) => setCommunityAlias(event.target.value)} placeholder="Pseudonym" aria-label="Community pseudonym" /><DuoSelect label="Post mood" value={moodState} onChange={setMoodState} options={(['HAPPY', 'CALM', 'NEUTRAL', 'ANXIOUS', 'SAD', 'DISTRESSED'] as const).map((mood) => ({ value: mood, label: moodLabel(mood) }))} /></div><RichTextEditor ref={communityEditorRef} placeholder="Share anonymously... you are safe here." onChange={(_, text) => setCommunityPost(text)} />
          {communityImage && (
